...
import org.crazyzhang.blog.utils.DateUtil;
...
//    }
     ...
     * 根据日期显示分类文章
     */
    @RequestMapping("/date/{id}/{date}")
    public String showPostByDate(@PathVariable("date") String date,@PathVariable("id") Integer id, Model model) {
        TwoDate twoDate = DateUtil.praseDate(date);
        if (twoDate != null) {
            List<Post> posts = postService.showPostByDate(id,twoDate);
            PageInfo page = new PageInfo(posts);
